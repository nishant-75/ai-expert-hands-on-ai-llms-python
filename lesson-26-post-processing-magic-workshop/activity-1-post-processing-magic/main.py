...
def generate_image_from_text(prompt):
    ...
    for model in MODELS:
        ...
        for _ in range(3):
            ...
            # Error — capture message and try next model
            try:
...
def post_process_image(image):
    image = ImageEnhance.Brightness(image).enhance(1.2)   # +20% brightness
    image = ImageEnhance.Contrast(image).enhance(1.3)     # +30% contrast
    return image.filter(ImageFilter.GaussianBlur(radius=2))  # soft-focus
