...
# Each finger toggles between two filters
pairs = {"middle": ("SEPIA", "NEGATIVE"), "ring": ("BLUR", "GLITCH"), "pinky": ("EDGE", "CARTOON")}
st = {k: 0 for k in pairs}
cur = "SEPIA"

DEB, CAP, TT, TP = 0.6, 1.2, 30, 20   # debounce, capture cooldown, tap threshold, pinch threshold
la = lc = 0
pinch_on = False
...
while True:
    # Pause / preview mode after capture
    if paused:
        cv2.imshow(MAIN, freeze)
        k = cv2.waitKey(50) & 0xFF
        if k == ord("q"):
            break
        if k == 27:  # ESC resumes live mode
            paused = False
            pinch_on = False
            try:
    ...
    if res.multi_hand_landmarks:
        ...
        # Thumb near middle/ring/pinky = switch filter
        if not pinch:
