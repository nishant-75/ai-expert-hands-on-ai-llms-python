...
def detect_gesture(hand_landmarks):
    ...
    # Other four fingers: check vertical movement
    for i in range(1, 5):
...
while True:
    ...
    if results.multi_hand_landmarks and results.multi_handedness:
        for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
            ...
            # Draw purple circles and yellow numbers on fingertips
            ...
            # Display Left/Right hand label near wrist
            wrist = hand_landmarks.landmark[0]
            wrist_x, wrist_y = int(wrist.x * w), int(wrist.y * h)
            cv2.putText(frame, f"{hand_label} Hand", (wrist_x - 40, wrist_y + 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

    # Green for clear gestures, orange for partial/unknown
