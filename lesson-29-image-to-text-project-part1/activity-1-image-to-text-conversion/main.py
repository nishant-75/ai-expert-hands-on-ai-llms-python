...
# Generates a basic image caption and offers 5/30/50 word outputs
# Requires: requests, pillow, colorama, config.py (HF_API_KEY)
...
def generate_text(prompt: str, max_new_tokens: int = 220) -> str:
    txt, err = _run_models(TEXT_MODELS, [{"role": "user", "content": prompt}],
                           max_tokens=max_new_tokens, temperature=0.4)
    if not txt:
        raise Exception(err)
    return txt
...
def generate_exact_sentence(prompt: str, n_words: int, max_new_tokens: int, tries: int = 6) -> str:
    for attempt in range(tries):
        txt = generate_text(prompt, max_new_tokens)
        w = _words(txt)
        if len(w) >= n_words:
            return _ensure_sentence_end(_exact_n_words(txt, n_words))
        if attempt < tries - 1:
            time.sleep(0.5)
    return _ensure_sentence_end(_exact_n_words(txt, n_words))
