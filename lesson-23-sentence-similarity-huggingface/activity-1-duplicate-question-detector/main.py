...
def smart_score(base, q1, q2, strong):
    ...
    oppA = [("increase", "decrease"), ("bigger", "smaller"), ("more", "less"),
            ("add", "remove"), ("open", "close"), ("enable", "disable")]
    num_pen = 0.10 if (nums(q1) and nums(q2) and nums(q1) != nums(q2)) else 0
    neg_pen = 0.12 if has_any(q1, negA) != has_any(q2, negA) else 0
    opp_pen = 0.12 if any(
        (has_any(q1, [a]) and has_any(q2, [b])) or (has_any(q1, [b]) and has_any(q2, [a]))
        for a, b in oppA
    ) else 0
...
def show_result(s):
    print(f"\n🎯 Similarity: {round(s * 100, 1)}% [{bar(s)}]  →  {label(s)}")
    print(f"Rule: score ≥ {TH} means DUPLICATE")

def show_flow(q1, q2):
    a, b = clean(q1), clean(q2)
    raw = set(a + b)
    w1 = {w for w in a if len(w) >= 4}
    w2 = {w for w in b if len(w) >= 4}
    shared = sorted(w1 & w2)
    ...
    print("\n4) Why similarity is high/low")
    if shared:
        print("   - Direct matches:", ", ".join(shared))
    else:
        print("   - The model used overall meaning patterns, not exact word matches.")
